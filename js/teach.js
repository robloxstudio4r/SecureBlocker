...
async function closeTab(sessionId, studentId, tabId, url) {
  ...
  if (error) console.error('[roster] closeTab failed:', error);
}
...
async function focusTab(sessionId, studentId, tabId) {
  ...
  if (error) console.error('[roster] focusTab failed:', error);
}
async function requestScreenshot(sessionId, studentId) {
  ...
  if (error) console.error('[roster] requestScreenshot failed:', error);
}
async function lock(id) {
  const { error } = await supabase.from('sessions').update({
    status: 'locked', lock_message: 'Focus on the lesson.'
  }).eq('id', id);
  if (error) { console.error('[roster] lock failed:', error); alert('Could not lock: ' + error.message); }
}
async function unlock(id) {
  const { error } = await supabase.from('sessions').update({ status: 'active' }).eq('id', id);
  if (error) { console.error('[roster] unlock failed:', error); alert('Could not unlock: ' + error.message); }
}
