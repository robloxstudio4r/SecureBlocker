...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'START_POLLING') { startPolling(); sendResponse({ ok: true }); return true; }
});
...
async function pollCommands() {
  const cfg = await chrome.storage.local.get(['supabaseUrl', 'anonKey', 'sessionId', 'accessToken']);
  if (!cfg.supabaseUrl || !cfg.sessionId || !cfg.accessToken) return;

  try {
    const res = await fetch(
      `${cfg.supabaseUrl}/rest/v1/tab_commands?target_session_id=eq.${cfg.sessionId}&executed_at=is.null&select=*`,
...
chrome.alarms.onAlarm.addListener((a) => { if (a.name === 'keepalive') pollCommands(); });

chrome.storage.local.get(['sessionId'], (cfg) => { if (cfg.sessionId) startPolling(); });
